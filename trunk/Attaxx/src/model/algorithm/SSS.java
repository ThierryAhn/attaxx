...
public class SSS implements PlayerAlgo {
	...
	private Move sssAlgo(AttaxxModel model){
			...
			if (!n.isResolved()){
				...
				if(n.getModel().getCurrentPlayer().equals(MIN)){
					...
					if(getRightBrother(n) != null){
						...
						System.out.println("pas de frère");
		...
		bestMove = listG.iterator().next().getMove();
		return bestMove;
	}
	...
	private NodeSSS extractFirst(){
		NodeSSS first = listG.iterator().next();
		listG.remove(first);
		return first;
	}
